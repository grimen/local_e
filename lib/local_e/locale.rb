module LocalE
  module Locale
    ...
    module InstanceMethods
      ...
      EXTRACT_LOCALE_METHODS = [:params, :session, :tld, :subdomain, :header, :ip, :default].freeze
      ...
      def extract_locale_from_params(only_available_locales = false)
        parsed_locale = filter_locale(self.params_value(:locale), only_available_locales)
      ...
      def extract_locale_from_session(only_available_locales = false)
        parsed_locale = filter_locale(self.session_value(:locale), only_available_locales)
      ...
      def extract_locale_from_header(only_available_locales = false)
        parsed_locale = filter_locale(self.http_accept_language, only_available_locales)
      ...
      def valid_format_of_locale?(value)
        value.blank? ? false : (value =~ /^[a-z]{2}(-[A-Z]{2}){0,1}$/)
      end
